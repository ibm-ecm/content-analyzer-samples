...
def registration(config):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                response1 = requests.request("POST", ums_url, headers=headers, data=json.dumps(payload),verify=config['ssl_verification'])
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if res:
                        ...
                        token, generated_time = generateToken(config)
                        return token, generated_time
                else:
